...
interface ResetPasswordHtmlInput {
  email: string;
  token: string;
}
...
export const getResetPasswordHtml = ({
  email,
  token,
}: ResetPasswordHtmlInput): string => {
  const app_name = process.env.APP_NAME || "Authentication App";
  const base_url = process.env.FRONTEND_URL || "http://localhost:5173";
  const reset_url = `${base_url.replace(/\/+$/, "")}/reset-password/${encodeURIComponent(token)}`;

  const safe_app_name = escape_html(app_name);
  const safe_email = escape_html(email);
  const safe_reset_url = escape_html(reset_url);
...
<meta name="viewport" content="width=device-width,initial-scale=1" />
<meta name="x-apple-disable-message-reformatting" />
<title>${safe_app_name} Reset Your Password</title>
<style>
html, body { margin: 0; padding: 0; }
body {
background: #f6f7fb;
...
<p class="text">
We received a request to reset the password for the account associated with ${safe_email}.
</p>
<p class="text">
...
<tr>
<td align="center">
<a class="btn" href="${safe_reset_url}" target="_blank" rel="noopener">Reset Password</a>
</td>
</tr>
</table>
<p class="muted">
...
<p class="muted">
<a class="link" href="${safe_reset_url}" target="_blank" rel="noopener">${safe_reset_url}</a>
</p>
<p class="muted">
